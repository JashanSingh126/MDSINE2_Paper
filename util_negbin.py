...
def build_synthetic_subjset(params):
    '''Build the `pylab.base.SubjectSet` object used to store the data
    ...
    Returns
    -------
    pylab.base.SubjectSet, pylab.base.SubjectSet
    ...
    subjset_main_inference = pl.base.SubjectSet.load(params.MAIN_INFERENCE_SUBJSET_FILENAME)
    subjset = pl.base.SubjectSet(asvs=subjset_main_inference.asvs)

    # Ground truth object
    subjset_true = pl.base.SubjectSet(asvs=subjset_main_inference.asvs)
...
def _build_subject_single_qPCR(subjectset, counts, qpcr_measurement, name=None):
    ...
    and adds it to the `pylab.base.SubjectSet` object `subjset`.
    ...
    Parameters
    ----------
    subjectset : pylab.base.SubjectSet
    ...
    pylab.base.SubjectSet
    '''
    if not pl.issubjectset(subjectset):
        raise TypeError('`subjectset` ({}) must be a pylab.base.SubjectSet'.format(
            type(subjectset)))
...
def filter_out_zero_asvs(subjset):
    ...
    if not pl.issubjectset(subjset):
        raise TypeError('`subjset` ({}) must be a pylab.base.SubjectSet object'.format(
            type(subjset)))
