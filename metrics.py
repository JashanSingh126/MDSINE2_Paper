...
class Metrics(pl.Saveable):
    ...
    def lookahead(self, time_lookahead, subject, dtype='abs', percentile=25, truth=None, 
        error_metric='pe'):
        '''Calculate the time lookahead predictions for the times indicated.

        We only save the errors

        Parameters
        ----------
        time_lookahead : numeric, array
            These are the time look-aheads to calculate
        subjset : pylab.base.Subject
            Subject we are validating on
        dtype : str
            type of data we are simulating on. Options:
        ...
        error_metric : callable, str
            ...
            Options:
                ...
                'rmse'
                    Root mean squared error (pylab.metrics.RMSE)
        '''
        if not pl.isarray(time_lookahead):
            time_lookahead = [time_lookahead]
        for ele in time_lookahead:
            if not pl.isnumeric(ele):
                raise TypeError('Every element in `time_lookahead` ({}) must be a numeric ({})'.format(
                    type(ele), time_lookahead))
            if ele <= 0:
                raise ValueError('All elements in `time_lookahead` ({}) must be  > 0 ({})'.format(
                    ele, time_lookahead))
        ...
        self.time_lookaheads[subject.name] = {}
        self.time_lookaheads[subject.name]['subject'] = subject
        if truth is not None:
            times = truth.times
        else:
            times = subject.times

        max_t = int(np.ceil(np.max(times)))
        self.time_lookaheads[subject.name]['lookaheads'] = {}
        for t in time_lookahead:
            self.time_lookaheads[subject.name]['lookaheads'][t] = {}
            self.time_lookaheads[subject.name]['lookaheads'][t]['pred'] = np.zeros(max_t)*np.nan
            self.time_lookaheads[subject.name]['lookaheads'][t]['times'] = np.arange(max_t)
            self.time_lookaheads[subject.name]['lookaheads'][t]['metric'] = error_metric
            if pl.isMCMC(self.model):
                self.time_lookaheads[subject.name]['lookaheads'][t]['low'] = np.zeros(max_t)*np.nan
                self.time_lookaheads[subject.name]['lookaheads'][t]['high'] = np.zeros(max_t)*np.nan
        ...
        for start, start_pnt_idx in enumerate(master_ts):
            ...
            pred_M = ret['pred-traj']

            if pl.isMCMC(self.model):
                high_M = ret['pred-high']
                low_M = ret['pred-low']


            if truth is not None:
                data_ts = ret['truth-times']
                data_M = ret['truth-traj']
            else:
                data_ts = ret['subj-times']
                data_M = ret['subj-traj']


            rev_idx_pred = {}
            for tidx,t in enumerate(pred_ts):
                rev_idx_pred[t] = tidx
            rev_idx_data = {}
            for tidx,t in enumerate(data_ts):
                rev_idx_data[t] = tidx

            # For each time lookahead calculate the different.
            # If the lookahead does not exist in either the predicted or
            # the data then we set it as nan (skip)
            logging.info('{}/{}'.format(start_pnt_idx, len(master_ts)))
            for tla in time_lookahead:
                t = tla + start
