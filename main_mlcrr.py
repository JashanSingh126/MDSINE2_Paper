...
def run(params, graph_name, subjset):
    ...
    if subjset.perturbations is not None:
        for pidx, subj_pert in enumerate(subjset.perturbations):
            perturbation = pl.contrib.Perturbation(start=subj_pert.start, 
                ...
                name=STRNAMES.PERT_VALUE + '_{}'.format(pidx))
    ...
    # are always negative. interactions and perturbations are unbounded
    diag = np.append(np.full(n_asvs, -1), np.ones(n_asvs*n_asvs))
    if subjset.perturbations is not None:
...
if __name__ == '__main__':
    ...
    # Generate parameters
    synparams = config.SimulationConfig(times=-1, 
        n_replicates=args.n_replicates,
        n_asvs=args.n_asvs, healthy=True, 
    ...
    params = config.MLCRRConfig(output_basepath=args.basepath, data_seed=args.data_seed,
        ...
        measurement_noise_level=args.measurement_noise_level, 
        qpcr_noise_level=args.qpcr_noise_level, n_cpus=args.n_cpus)
    ...
    if not ONLY_PLOT:
        ...
        if not LOAD_SYNDATA_FROM_PKL:
            ...
            # subjset_real = filtering.avg_counts(subjset_real, fparams)
            # subjset_real = filtering.consistency(subjset_real, fparams)
        
            # Get the last 5 days
            ss = subjset_real.iloc(0).matrix()['abs']
            ss = np.mean(ss[:,-5:], axis=1)
            ss = ss[ss > 0]
            logging.info('Number of steady-states: {}'.format(len(ss)))

            # Sample the perturbations of the dynamics
            syndata = synthetic.SyntheticData(log_dynamics=True)
            syndata.icml_topology(n_asvs=synparams.N_ASVS, 
                max_abundance=synparams.MAX_ABUNDANCE)
            if synparams.PERTURBATIONS is not None:
                for start, end, probability_positive, strength, probability_affect in synparams.PERTURBATIONS:
                    syndata.sample_single_perturbation( 
                        start=start, end=end, prob_pos=probability_positive, 
                        strength=strength, prob_affect=probability_affect, scalar=False)
        ...
        subjset = syndata.simulateRealRegressionDataNegBin(subjset=subjset_real,
            a0=synparams.NEGBIN_A0, a1=synparams.NEGBIN_A1, 
            qpcr_noise_scale=synparams.QPCR_NOISE_SCALE)
    ...
    noise_subjset, exact_subjset = main_ICML.make_val_subjset(syndata_filename=syndata_filename, 
        synparams_filename=synparams_filename, params_filename=params_filename)
