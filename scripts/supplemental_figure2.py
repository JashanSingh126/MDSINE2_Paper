...
def get_top_200_abundance_data(subjset_he, subjset_uc):
    """
    ...
    top_200_idxs = idxs[0:200]
    print(top_200_idxs)
    sorted_idxs = np.sort(top_200_idxs)
    final_li = ["OTU_" + str(i + 1) for i in sorted_idxs]

    np_healthy_200 = combine_dfs(subjset_he, "abs").to_numpy()[top_200_idxs]
    np_uc_200 = combine_dfs(subjset_uc, "abs").to_numpy()[top_200_idxs]

    df_uc = pd.DataFrame(np_uc_200, index = final_li,
            columns = combined_df.columns)
    df_healthy = pd.DataFrame(np_healthy_200, index = final_li,
            columns = combined_df.columns)
    print(df_uc, df_healthy)
...
def select_rows(df, top_, names = None):
    """
       selects the rows associated with ASVs in top_

       @parameters
       ------------------------------------------------------------------------
       df : (pandas DataFrame)
       top_ : ([str]) a list containing names of ASVs
    ...
    np_df = df.to_numpy()
    index = df.index
    index = [name.replace("_agg", "") for name in index]
    top_np = []
    new_index = []
    for i in range(np_df.shape[0]):
        ...
        id_ = index[i]
        if id_ in top_:
            top_np.append(list(np_df[i]))
            if names is None:
                new_index.append(id_.replace("_", " "))
            else:
                new_index.append(names[id_])

    array = np.asarray(top_np)
    top_df = pd.DataFrame(array, index = new_index, columns = df.columns)
    return top_df
...
def draw_heatmap(data_healthy, data_uc, uc_filter, healthy_filter, max_, min_,
    ...
    perturbation_start = [21.5, 35.5, 50.5]
    perturbation_end = [28.5, 42.5, 57.5]

    axes1 = add_perturbation_label(axes1, subjset.perturbations,subj, times,
            alpha=0, textsize = 35)
    for i in range(len(perturbation_start)):
        axes1.axvline(x = times.index(perturbation_start[i]), color = "black",
        linestyle = '-', lw=2.5)
        axes1.axvline(x = times.index(perturbation_end[i]), color = "black",
        linestyle = '-', lw=2.5)

    axes2 = add_perturbation_label(axes2, subjset.perturbations, subj, times,
            alpha=0, textsize = 35)
    for perturbation in subjset.perturbations:
        axes2.axvline(x = times.index(perturbation.starts[subj.name]), color = "black",
        linestyle = '-', lw=2.5)
        axes2.axvline(x = times.index(perturbation.ends[subj.name]), color = "black",
        linestyle = '-', lw=2.5)
...
def get_sample_times(subjset):
    """
    returns the time points at which samples were taken
...
def get_abundances(subjset):

    df = combine_dfs(subjset, "abs")
    print(df)
    return df
...
def main():
    ...
    args = parse_args()
    loc = "../figure_files/figure2/"
    save_path = "output_figures/"
    #top 400 most abundant species

    uc_filename = "uc_abund.tsv"
    healthy_filename = "healthy_abund.tsv"
    uc_filter_file = "uc_7.txt"
    healthy_filter_file = "healthy_7.txt"
    ...
    top_400 = get_top_200_abundance_data(subjset_healthy, subjset_uc)
    #df_uc = get_abundances(subjset_healthy)
    """
    df_uc = pd.read_csv(loc + uc_filename, sep = "\t", header = "infer",
        index_col= 0)
    df_healthy = pd.read_csv(loc + healthy_filename, sep = "\t", header = "infer",
        index_col= 0)

    names_uc = get_names(df_uc, subjset_uc)
    names_healthy = get_names(df_healthy, subjset_healthy)

    df_uc_200 = select_rows(df_uc, top_400[0:200])
    df_healthy_200 = select_rows(df_healthy, top_400[0:200], names = names_healthy)

    uc_pass_filtering = get_filtered_otu_names(loc + uc_filter_file)
    healthy_pass_filtering = get_filtered_otu_names(loc + healthy_filter_file)

    max_ = get_max(df_uc_200.to_numpy(), df_healthy_200.to_numpy())
    min_ = 1e4
    legend = "Taxonomy Key \n* Genus, ** : Family, *** : Order, **** : Class,"\
              " ***** : Phylum, ****** : Kingdom"

    draw_heatmap(df_healthy_200, df_uc_200, uc_pass_filtering,
                 healthy_pass_filtering, max_, min_, save_path + "top_200_7",
