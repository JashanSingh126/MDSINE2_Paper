...
def alpha_diversity_mean_std(subjset_healthy, subjset_uc, subjset_innoc, name,
    ...
    md2.visualization.shade_in_perturbations(ax, perturbations =
    subjset_healthy.perturbations, textsize=18, alpha=0)
    for perturbation in subjset_healthy.perturbations:
        ax.axvline(x=perturbation.start, color='black', linestyle='--', lw=2)
        ax.axvline(x=perturbation.end, color='black', linestyle='--',
           linewidth=2)
...
def beta_diversity_figure(subjset_healthy, subjset_uc, subjset_innoc, name = None,
    ...
    for pidx in range(1,7):
        pert_name = subjset_healthy.perturbations[(pidx-1)//2].name
...
def diversity_plot(subjset_healthy, subjset_uc, subjset_innoc, name = None):
    ...
    fig.savefig("combined_.pdf", bbox_inches = "tight")
    plt.close()
...
def loaddata(healthy):
    ...
    pl.base.SubjectSet
    '''
    subjset = pl.base.SubjectSet.load('../gibson_output/datasets/real_subjectset.pkl')
    # subjset.pop_times([0])
...
def main():
    ...
    #subjset_innoc = pl.base.SubjectSet.load('../gibson_output/datasets/inoculum_subjectset.pkl')
    ...
    #alpha_diversity_mean_std(subjset_healthy, subjset_uc, subjset_innoc, "non_filtered_")
    ...
    diversity_plot(subjset_healthy, subjset_uc, subjset_innoc, name = "filtered_")
