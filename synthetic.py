...
class SyntheticData(pl.Saveable):
    ...
    the internal representation into a `pylab.base.SubjectSet` so
    ...
    def simulateRealRegressionDataDMD(self, subjset, alpha, qpcr_scale=None):
        ...
        # load SubjectSet and filter
        if type(subjset) == str:
            subjset = pl.SubjectSet.load(subjset)
        elif not pl.issubjectset(subjset):
            raise ValueError('`subjset` ({}) must eb a pl.SubjectSet object'.format(
                type(subjset)))
    ...
    def simulateExactSubjset(self):
        ...
        pylab.Base.SubjectSet
        '''
        subjset = pl.SubjectSet(asvs=self.asvs)
        data = self.data
        times = self.times
