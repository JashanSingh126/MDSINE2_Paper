...
def simple_plot(df1, df2, cohort_name, level, loc, name):

    fig = plt.figure(figsize=(15, 20))
    spec = GridSpec(ncols=8, nrows=2, figure=fig)
    axes1 = fig.add_subplot(spec[0, 0:6])
    cax = fig.add_subplot(spec[0, 7:8])
    axes2 = fig.add_subplot(spec[1, 0:5])

    axes1 = plot_module(cohort_name, df1, axes1, "A", level, plot_cbar=True,
       cbar_ax=cax, tick_right=False )
    axes2 = _make_table(cohort_name, df2, axes2, "B", False, level)
    os.makedirs(loc, exist_ok=True)
