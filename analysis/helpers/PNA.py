...
def load_cocluster_data(saved_filename, union_d, flora_d, union_li, flora_li):
    """loads the co-cluster matrix and returns the matrix as numpy array

        @parameters
        saved_filename : name of the file where the co-cluster data is saved
        union_d : (dict) (str) name of otu -> (int) index
        flora_d : (dict) (str) name of otu -> (int) index
        union_li : [str] ids of otu present in the union
        flora_li : [str] ids of otu present in the given flora
    """

    N = len(union_d)
    cocluster_matrix_union = np.zeros((N, N))
    cocluster_matrix_flora = np.load(saved_filename, allow_pickle = True)
    cocluster_matrix_flora = np.nanmean(cocluster_matrix_flora, axis = 0)
    for i in range(N):
        otu_i = union_li[i]
        for j in range(N):
            otu_j = union_li[j]
            if otu_i in flora_d and otu_j in flora_d:
                prob = cocluster_matrix_flora[flora_d[otu_i], flora_d[otu_j]]
...
def agglomerative_clustering_all(pi_mat, order_d, otu_li, stop_criteria = 1):


    unique_pi = {}
    active_set = {}
    all_ = {len(active_set) : [active_set, 100]}
    unique_pi = {len(active_set) : active_set}
    for i in range(len(otu_li)):
        active_set[i] = [otu_li[i]]

    count = len(otu_li)
    while len(active_set) != stop_criteria:
        min_d, arg_min_d1, arg_min_d2 = linkage_info(active_set, pi_mat, order_d)
        #print(active_set[arg_min_d1], active_set[arg_min_d2]
        active_set = reorder_active_set(active_set, arg_min_d1, arg_min_d2, count)
        if  min_d not in unique_pi:
            unique_pi[min_d] = rename_set(active_set)
            all_[len(active_set)] = [rename_set(active_set), min_d]
        count += 1
