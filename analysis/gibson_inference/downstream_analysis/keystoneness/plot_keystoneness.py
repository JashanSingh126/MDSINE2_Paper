...
def parse_args():
    ...
    parser.add_argument('--fixed-cluster-mcmc-path', type=str, dest='input_mcmc', required=True,
    ...
    parser.add_argument('--dataset-name', type=str, dest='dataset_name', required=True,
                        help='<Required> The name of the dataset.')
    parser.add_argument('--subjset-path', type=str, dest='subjset_path', required=True,
                        help='<Required> The path to the subjset pickle.')
    parser.add_argument('--fwsim-df-path', type=str, dest='fwsim_df_path', required=True,
                        help='<Required> The path the HDF5 file containing the result of forward simulations.')
...
class KeystonenessFigure():
    def __init__(self, dataset_name: str, mcmc_pickle_path, subjset_path, fwsim_path):
        ...
        self.md: MdsineOutput = MdsineOutput(dataset_name, mcmc_pickle_path)
        self.study = md2.Study.load(subjset_path)

        print("Loading dataframe from disk.")
        self.fwsim_df: pd.DataFrame = pd.read_hdf(fwsim_path, key='df', mode='r')
    ...
    def plot(self, fig):
        ...
        altered_array = self.abundance_array[1 + ky_order, :]  # Reorder the rows first (exclude the baseline row),
        ...
        # =========== Heatmap settings. ========
        gridspec_kw = {
            "height_ratios": [1, 1, self.abundance_array.shape[0] - 1],
            "width_ratios": [1, self.abundance_array.shape[1]]
        }
