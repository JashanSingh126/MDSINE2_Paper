...
def alternative_plot(data_df, title1, title2, use_log, bar_df, type_, axes1,
   axes2, y_lab, donor, add_legend=False):
    ...
    if type_ =="rel":
        axes1.set_xlabel("$log_{10}$ (Mean Rel Abundance)", fontsize=AXES_FONTSIZE,
           labelpad=3)
        axes2.set_xlabel("$log_{10}$ (Mean Rel Abundance)", fontsize=AXES_FONTSIZE)
        axes1.set_ylim(0, 0.65)
        axes2.set_ylim([0, 5])

    else:
        #print("abs setting x label")
        axes1.set_xlabel("$\log_{10}$ (Mean Abs Abundance)", fontsize=AXES_FONTSIZE
            , labelpad=3)
        axes2.set_xlabel("$\log_{10}$ (Mean Abs Abundance)", fontsize=AXES_FONTSIZE)
    ...
    axes2.set_ylabel(y_lab, fontsize=AXES_FONTSIZE)
...
def main():
    ...
    alternative_plot(healthy_abs_alt_df, "A", "C", use_log, healthy_abs_abund_df,
       "abs", ax_he_abs_hist, ax_he_abs_bin, "RMSE($\log_{10}$ Abs Abundance)",
       "healthy")
    alternative_plot(uc_abs_alt_df, "B","D", use_log, uc_abs_abund_df,
        "abs", ax_uc_abs_hist, ax_uc_abs_bin, "RMSE ($\log_{10}$ Abs Abundance)",
        "uc", add_legend=True)

    alternative_plot(healthy_rel_alt_df, "E", "G", use_log, healthy_rel_abund_df,
       "rel", ax_he_rel_hist, ax_he_rel_bin, "RMSE ($\log_{10}$ Rel Abundance)",
        "healthy")
    alternative_plot(uc_rel_alt_df, "F", "H", use_log, uc_rel_abund_df,
        "rel", ax_uc_rel_hist, ax_uc_rel_bin, "RMSE ($\log_{10}$ Rel Abundance)",
        "uc", add_legend=True)

    os.makedirs(args.output_path, exist_ok=True)
    fig.savefig(args.output_path+"figure3_supp.pdf", bbox_inches="tight",
        dpi=800)
