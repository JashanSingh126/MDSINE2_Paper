"""
python supplemental_figure_8_and_9.py -file1 "../files/figures/healthy_coclusters.tsv"\
...
def parse_args():
    ...
    parser.add_argument("-file7", "--healthy_bayes", required = "True",
    help = ".tsv file containing bayes factors for healthy cohort")
    parser.add_argument("-file8", "--uc_bayes", required = "True",
    help = ".tsv file containing bayes factors for UC cohort")
    parser.add_argument("-opt", "--enable_opt", required = "True",
    help = ".tsv file containing consensus cluster assignment for UC cohort")

    return parser.parse_args()

def plot_bayes_fac_heatmap(data, x_names, y_names, figsize_x, figsize_y,
...
def bayes_factor(bayes_fac, x_names, y_names, otus_order, index_d,
    name):
    """
       ...
       @parameters
       ------------------------------------------------------------------------
       bayes_fac : (np.array) Bayes Factor
    ...
    """
    print("Bayes Factor Heatmap ")

    mat_reordered = helper.reorder(bayes_fac, otus_order, index_d)
    plot_bayes_fac_heatmap(mat_reordered, x_names, y_names,
         24, 27, 17, 14, name + "_bayes_fac")
...
def main():
    ...
    healthy_bayes_fac = pd.read_csv(args.healthy_bayes, sep = "\t",
                        index_col = 0)
    healthy_order_li = list(healthy_bayes_fac.columns)
    healthy_order_d = {healthy_order_li[i] : i for i in range(len(healthy_order_li))}
    print("Making Figure 8")
    bayes_factor(healthy_bayes_fac.to_numpy(), x_healthy, y_healthy,
    ...
    uc_bayes_fac = pd.read_csv(args.uc_bayes, sep = "\t",
                        index_col = 0)
    uc_order_li = list(uc_bayes_fac.columns)
    uc_order_d = {uc_order_li[i] : i for i in range(len(uc_order_li))}
    print("Making Figure 8")
    bayes_factor(uc_bayes_fac.to_numpy(), x_uc, y_uc,
