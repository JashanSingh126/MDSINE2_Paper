...
def obtain_p_vals(cluster_dict, hierarchy_otu_dict):
    ...
    for id_ in sorted(cluster_dict.keys()):
        ...
        for level in hierarchy_otu_dict:
            ...
            #print(total_otus, n_otus_annotated, n_otus_cluster,
            #    n_otus_annotated_cluster )
            #p_val = compute_p_value(total_otus, n_otus_annotated, n_otus_cluster,
            #    n_otus_annotated_cluster)
            #cluster_all_p[id_].append(p_val)
            #cluster_all_level[id_].append(level)
...
def pivot_df(p_li, p_info, adjusted_p_li, adjusted_p_res_li, name):
    ...
    #print("Pivoting DF", len(p_li), len(p_info), len(adjusted_p_li))
    ...
    for i in range(len(p_li)):
        if adjusted_p_res_li[i] == True:
            #print(adjusted_p_li[i], p_info[i])
...
def make_plot(df_healthy_p_order, df_uc_p_order, df_healthy_p_family,
    df_uc_p_family, df_healthy_p_class, df_uc_p_class, df_healthy_order_abund,
    df_uc_order_abund, df_healthy_family_abund, df_uc_family_abund,
    df_healthy_class_abund, df_uc_class_abund):

    fig = plt.figure(figsize=(40, 55))
    spec = GridSpec(ncols=44, nrows= 80, figure=fig)
    ...
    family_enrich_start = 59
    family_enrich_end = 62

    order_enrich_start = 65
    order_enrich_end = 71

    class_enrich_start = 74
    class_enrich_end = 80
    ...
    ax_healthy_family_enrich = plot_module(df_healthy_p_family, ax_healthy_family_enrich,
        "G", "Family", plot_cbar=False)
    ax_uc_family_enrich = plot_module(df_uc_p_family, ax_uc_family_enrich, "H",
    ...
    ax_healthy_order_enrich = plot_module(df_healthy_p_order, ax_healthy_order_enrich,
        "I", "Order", plot_cbar=False)
    ax_uc_order_enrich = plot_module(df_uc_p_order, ax_uc_order_enrich, "J",
    ...
    ax_healthy_class_enrich = plot_module(df_healthy_p_class, ax_healthy_class_enrich,
        "K", "Class", plot_cbar=False)
    ax_uc_class_enrich = plot_module(df_uc_p_class, ax_uc_class_enrich, "L",
