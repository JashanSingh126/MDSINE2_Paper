...
#for each gibbs step count the number of eigenvalues that are unstable
...
a = ax.hexbin(X,Y, bins='log', cmap='inferno', mincnt=2, linewidths=0.2)
ax.set_xlabel('Real')
ax.set_ylabel('Imaginary')
cb = fig.colorbar(a, ax=ax)
#ax.ylim([-1e-8, 1e-8])

#plt.savefig('eiganvalue_heatmap_uc.pdf')
#plt.xlim([-1e-11, 0])

#plt.show()

arr = np.load(healthy_filename)
print(arr.shape)
...
plt.savefig('eiganvalue_heatmap_both.pdf')


#fig = plt.figure()
#sns.kdeplot(X,Y)

#plt.savefig('eiganvalue_contour_healthy.pdf')
