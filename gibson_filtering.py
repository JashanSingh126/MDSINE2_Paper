...
1) Load in dataset
   ...
   `MDSINE2.gibson_preprocessing.py` script. To quickly generate this pickle:
    ```python
    import mdsine2
...
Reproducability
---------------
To reproduce the paper, first run the script `gibson_preprocessing.py`, then:
python gibson_filtering.py \
    --dataset gibson_output/gibson_healthy_agg.pkl gibson_output/gibson_uc_agg.pkl \
    --other gibson_output/gibson_uc_agg.pkl gibson_output/gibson_healthy_agg.pkl \
    --outfile gibson_output/gibson_healthy_agg_filtered.pkl gibson_output/gibson_uc_agg_filtered.pkl \
    --filtering conditional_consistency_filtering \
    --dtype rel \
    --threshold 0.0001 \
    --min-num-consecutive 7 \
    --min-num-consecutive-lower 5 \
    --min-num-subjects 2 \
    --colonization-time 5
'''
import argparse
